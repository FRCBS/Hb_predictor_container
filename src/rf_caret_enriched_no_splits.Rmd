...
source("~/FRCBS/Hb_predictor_container/src/helper_functions.R")
...
save_figs <- FALSE
compute <- FALSE
result_dir <- "~/FRCBS/interval_prediction/oversampling_results"
...
train_orig <- load_single(file="../data/preprocessed-2020-08-26-train.rdata")
#train_orig <- sample_set(train_orig, 0.5)
#enriched <- load_single(file="../data/preprocessed-2020-08-26-train-enriched.rdata")
validate_orig <- load_single(file="../data/preprocessed-2020-08-26-validate.rdata")
test_orig <- load_single(file="../data/preprocessed-2020-08-26-test.rdata")
...
```{r Visualize the decision tree, eval=FALSE}
rpart.plot(trtree,type = 4, extra = 101, tweak = 1)
```

```{r Save the above decision tree, eval=FALSE}
if (save_figs) {
...
```{r Visualize another tree, eval=FALSE}
rpart.plot(trtree,type = 1, extra = 9, tweak = 1)
```

```{r Save the above visualization of the decision tree, eval=FALSE}
...
```{r, eval=FALSE}
pred.c <- predict(trtree, validate, type = "class")
...
```{r create_performance_plots_dtree, eval=FALSE}
#https://stackoverflow.com/questions/30818188/roc-curve-in-r-using-rpart-package
pred.p <- predict(trtree, validate, type = "prob")
pred.p.test <- predict(trtree, test, type = "prob")
...
df <- data.frame(deferral=as.integer(validate$Hb_deferral) - 1, scores=pred.p[,'Deferred'])
...
if (save_figs) {
  gsave(filename=filename, varimp.plot, width = 180,  height = 80,units="mm", dpi=600, scale=1.0)
}
varimp.plot
```
...
## Effect of class assingment probabilty
