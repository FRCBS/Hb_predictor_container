...
stuff <- list(fit, enriched, test)
saveRDS(stuff, sprintf("/tmp/baseline-stuff-%s.rds", params$sex))
```

```{r Predict validate data}
predict_baseline <- function(glm.fit, test) {
  ...
  result <- tibble(score = glm.probs, 
                   ...
                   predicted_label = as.integer(score > 0.5),
...
test_result <- predict_baseline(fit, test)
```
...
plot_logistic_regression <- function(df, title) {
  g <- df %>% 
    ggplot(aes(x=previous_Hb, y=score)) + 
    geom_line() +
    geom_point(mapping=aes(x=previous_Hb, y=original_label, color=sex)) +
    labs(y="Probability", title=title)
  return(g)
}
enriched_g <- plot_logistic_regression(enriched_result, "enriched data")
test_g <- plot_logistic_regression(test_result, "test data")
enriched_g
test_g
```
