...
# simple.model <- paste0(basepath, "src/stan/", "mem_qr_1_noa_train.stan")
# simple.model.consts <- paste0(basepath, "src/stan/", "mem_qr_1_noa_consts.stan")
# icp.model <- paste0(basepath, "src/stan/", "mem_qr_1_heckman.stan")
# icp.model.consts <- paste0(basepath, "src/stan/", "mem_qr_1_heckman_consts.stan")
...
# Parameter names for different models
# icp, simple, fd_icp and fd_icp with consts
#params.icp <- c("beta[1]", "beta[2]", "beta[3]", "beta[4]","beta[5]","beta[6]","beta[7]","beta[8]","beta[9]","beta[10]","ups[1]","ups[2]","ups[3]","ups[4]")
params.icp <- c(sprintf("beta[%i]", 1:10), sprintf("ups[%i]", 1:4))

#params.simple <- c("beta[1]", "beta[2]", "beta[3]", "beta[4]","beta[5]","beta[6]","beta[7]","beta[8]","beta[9]","beta[10]")
params.simple <- sprintf("beta[%i]", 1:10)

#fd.params.icp <- c("beta[1]", "beta[2]", "beta[3]", "beta[4]","beta[5]","beta[6]","beta[7]","beta[8]","beta[9]","beta[10]","beta[11]","beta[12]","beta[13]","beta[14]","beta[15]","beta[16]","beta[17]","beta[18]","beta[19]","beta[20]","beta[21]","ups[1]","ups[2]","ups[3]","ups[4]")
fd.params.icp <- c(sprintf("beta[%i]", 1:21), sprintf("ups[%i]", 1:4))

#fd.params.icp.consts <- c("beta[1]", "beta[2]", "beta[3]", "beta[4]","beta[5]","beta[6]","beta[7]","beta[8]","beta[9]","beta[10]","beta[11]","beta[12]","beta[13]","beta[14]","beta[15]","beta[16]","beta[17]","beta[18]","beta[19]","beta[20]","beta[21]","phi[1]","phi[2]","phi[3]","phi[4]","phi[5]","phi[6]", "ups[1]","ups[2]","ups[3]","ups[4]")
...
#new.variables = c("Eryt", "HKR", "Leuk", "Trom", "MCH", "MCHC", "MCV", "RDW", "CRP", "Ferritin", "TransferrinR")
#additional.variables = c("BMI","smoking_status", "physical_condition", "meat_amount", "sleep_quality", "iron_supplement")

#test.sl <- stan_preprocess_icp_new(folds.male[[1]] %>% select(-Hb_first), tolag = new.variables, donor_variables = additional.variables, test_data = FALSE)
#fd.icp.vars <- c(colnames(test.sl$x_train), paste(colnames(test.sl$Z), "first", sep="_"))
#fd.icp.vars.consts <- c(fd.icp.vars, colnames(test.sl$C))
load(paste(datapath, "variable_names.rdata", sep="/"))
#c("days_to_previous_fb", "previous_Hb_def", "age", "year", "warm_season", "hour", "consecutive_deferrals", "recent_donations", "recent_deferrals", "previous_Hb", "Eryt", "HKR", "Leuk", "Trom", "MCH", "MCHC", "MCV", "RDW", "CRP", "Ferritin", "TransferrinR", "age_first", "year_first", "warm_season_first", "hour_first")
...
#fd.icp.vars.consts <- c(fd.icp.vars, "BMI", "smoking_status", "physical_condition", "meat_amount", "sleep_quality", "iron_supplement")
...
#post.hm$posterior_plot
#post.hmc$posterior_plot
#combined.hm$comp_plot
#combined.hm$cm
#plot(combined.hm$roc)
#combined.hm$pr_plot

#ggsave(filename="~/performances-hm.pdf", combined.hm$performances, width = 180, units="mm", dpi=600, scale=1.5)
```

```{r}
#ggsave(filename=paste0(datapath, "figures/posterior_effects_findonor_heckman_male.pdf"), plot=post.hm$posterior_plot)
#ggsave(filename=paste0(datapath, "figures/posterior_effects_findonor_heckman_female.pdf"), plot=post.hf$posterior_plot)
#ggsave(filename=paste0(datapath, "figures/posterior_effects_findonor_heckman_male_consts.pdf"), plot=post.hmc$posterior_plot)
#ggsave(filename=paste0(datapath, "figures/posterior_effects_findonor_heckman_female_consts.pdf"), plot=post.hfc$posterior_plot)
```

```{r}
#ggsave(filename=paste0(datapath, "figures/scatter_plot_findonor_heckman_male.pdf"), plot=combined.hm$comp_plot)
#ggsave(filename=paste0(datapath, "figures/scatter_plot_findonor_heckman_female.pdf"), plot=combined.hf$comp_plot)
#ggsave(filename=paste0(datapath, "figures/scatter_plot_findonor_heckman_male_consts.pdf"), plot=combined.hmc$comp_plot)
#ggsave(filename=paste0(datapath, "figures/scatter_plot_findonor_heckman_female_consts.pdf"), plot=combined.hfc$comp_plot)
```

```{r}
# errors <- data.frame(fMAE = c(combined.hf$mean_errors[1,2], combined.hfc$mean_errors[1,2]), 
#                  fRMSE= c(combined.hf$mean_errors[2,2], combined.hfc$mean_errors[2,2]), 
#                  fAUC = c(combined.hf$roc_auc,          combined.hfc$roc_auc),
#                  mMAE = c(combined.hm$mean_errors[1,2], combined.hmc$mean_errors[1,2]), 
#                  mRMSE= c(combined.hm$mean_errors[2,2], combined.hmc$mean_errors[2,2]), 
#                  mAUC = c(combined.hm$roc_auc,          combined.hmc$roc_auc)
#                  )
#rownames(errors) <- c("Heckman", "Heckman consts")
mypath2="~/FRCBS/results-for-eba-project/tables/"
errors <- tibble(
  Data = "FinDonor",
...
write.csv(errors, file = paste0(mypath2, "findonor-errors-both-icp-fix.csv"), row.names = FALSE, quote=FALSE)
latex_errors <- kable(errors, format="latex", digits=3, caption="findonor-errors-both-icp-fix", label="findonor-errors-both-icp-fix",
                           linesep="")
errors
```
```{r}
#heckman_cis <- cbind(post.hf$cis, post.hm$cis)
#latex_heckman_cis <- kable(heckman_cis, format="latex", digits=3, caption="Effect sizes Heckman", label="effects-fd-h",
#                           linesep="")
#heckman_cis
```

```{r store scatter-confusion and performances}
mypath="~/FRCBS/results-for-eba-project/pdf/"
create_scatter_confusion_plots(combined.hfc$df, 125, filename=paste0(mypath, "findonor-scatter-confusion-female-icp-fix.pdf"))
create_scatter_confusion_plots(combined.hmc$df, 135, filename=paste0(mypath, "findonor-scatter-confusion-male-icp-fix.pdf"))

create_performance_plots(combined.hfc$df, filename=paste0(mypath, "findonor-performance-female-icp-fix.pdf"))
create_performance_plots(combined.hmc$df, filename=paste0(mypath, "findonor-performance-male-icp-fix.pdf"))
```

```{r store regression coefficients}
mypath="~/FRCBS/results-for-eba-project/pdf/"
ggsave(filename=paste0(mypath, "findonor-coefficient-male-icp-fix.pdf"), plot=post.hmc$posterior_plot, width = 180, units="mm")
ggsave(filename=paste0(mypath, "findonor-coefficient-female-icp-fix.pdf"), plot=post.hfc$posterior_plot, width = 180, units="mm")

mypath2="~/FRCBS/results-for-eba-project/tables/"
heckman_consts_cis <- cbind(post.hfc$cis, post.hmc$cis)[-5]
write.csv(heckman_consts_cis, file = paste0(mypath2, "findonor-coefficient-both-icp-fix.csv"))
latex_heckman_consts_cis <- kable(heckman_consts_cis, format="latex", digits=3, caption="findonor-coefficient-both-icp-fix", label="findonor-coefficient-both-icp-fix",
                           linesep="")
cat(latex_heckman_consts_cis, file = paste0(mypath2, "findonor-coefficient-both-icp-fix.tex"), sep = "\n")
```
