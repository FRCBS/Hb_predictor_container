...
if (length(rf_hyperparameters) == 0) {   # Hyperparameters not available for this model yet
  ...
  if (quick) {
    ...
    rf_grid <-  expand.grid(
      mtry = c(3:8),
      nodesize = seq(1, 39, 2),  # Minimum node size
      ntree = 500
  ...
  ret <- learn_hyperparameters(df = train, method = "rf", search_grid = rf_grid, cores = number_of_cores,
...
train    <- train    %>% mutate(across(intersect(c("warm_season", "previous_Hb_def"), names(train)), as.integer))   ## POISTA, vain testausta varten 
...
if (use_pdf) {
  ...
  set.seed(global_random_seed)
  n <- as.integer(sum(train$Hb_deferral == "Accepted") / 2)  # oversample to the count of the most common class
  ...
  rrfFit_roc <- caret::train(x = as.data.frame(train %>% select(-Hb_deferral)), y = train %>% pull(Hb_deferral),
                             ...
                             #method = "ranger", 
                             strata=train$Hb_deferral,
                             sampsize=n,  # I start to think that this doesn't do stratified sampling.
