...
#   variables <- setdiff(variables, "gender")
# }
...
if (nrow(train) <= rf_hyper_parameters$min.node.size) {
  ...
  warning(sprintf("There are less points in the train data (%i) than what is the min.node.size of the random forest(%i)",
                  nrow(train), rf_hyper_parameters$min.node.size))
}
...
rrfFit_roc <- caret::train(Hb_deferral ~ ., data = train, 
                 ...
                 ## to evaluate:
                 #tuneLength=1,
                 tuneGrid = do.call(expand_grid, rf_hyper_parameters),
                   # expand_grid(mtry = 4,
                   #  splitrule="hellinger",
                   #  min.node.size = 34),
                #metric="ROC",
...
message("Here6")
id <- sprintf("%s-rf", params$gender)
t <- tibble(Id=id, Model="Random forest", Gender=params$gender, 
