...
if (create_datasets_bool) {
  ...
  if (out_of_sample_predictions) {
    ...
    if (compute_dlmm) {
      ...
      sl.val.dlmm <- stan_preprocess_icp_new(drop_some_fields(test, params$sex) %>% select(-Hb_first),  
      ...
      saveRDS(sl.val.dlmm, stan_preprocessed_validate_filename)
      #stan_preprocessed_validate_objects <- c(stan_preprocessed_validate_objects, "sl.val.dlmm")
    }
    
    if (compute_lmm) {
      stan_preprocessed_validate_filename <- sprintf("%s/stan_preprocessed_validate_datasets_%s_%s.rds", datadir, "lmm", params$sex)
      sl.val.lmm <- stan_preprocess_new(drop_some_fields(test, params$sex) %>% select(-previous_Hb),  
                                        basic_variables=basic_variables, donor_variables = donor_variables, 
                                        test_data = FALSE,
                                        Hb_index=Hb_index)#, hlen=params$hlen, hlen_exactly=params$hlen_exactly)
      saveRDS(sl.val.lmm, stan_preprocessed_validate_filename)
#      stan_preprocessed_validate_objects <- c(stan_preprocessed_validate_objects, "sl.val.lmm")
    }
  
    #stan_preprocessed_validate_filename <- paste(datadir,"stan_preprocessed_validate_datasets_", id, ".RData", sep = '')
    #save(list=stan_preprocessed_validate_objects, file = stan_preprocessed_validate_filename)
...
run_linear_model <- function(Id, Model, Pretty, Sex, stan_model, stan_parameters, pretty_variables) {
  ...
  cat("1\n")
  message("1")
  stan_fit_file <- sprintf("%s/model_%s.rds", stanfitpath, fit_id)
  if (skip_train == TRUE) {
  ...
  if (out_of_sample_predictions) {
    ...
    if (Sex == "both") {
      test_donor_sex <- sl.val$x_train$sex[results$last_events]
    }
    sl.val$y_test <- sl.val$y_train[results$last_events]
  } else {
  ...
  if (out_of_sample_predictions) {
    ...
    msg <- sprintf("Train data F1 threshold is %.3f, FPR is %.3f, TPR is %.3f\n", f1_threshold, 1-cp$specificity, cp$sensitivity)
    cat(msg)
    message(msg)

    df <- create_result_dataframe(sl.val, prediction_matrix, Hb_cutoff, metric="mean", f1_threshold=f1_threshold)  
  } else {
