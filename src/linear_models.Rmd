...
pmale <- summary_plotter(pmale, descript, "blue")  
pmale
...
if (!is.null(params$donor_specific_file)) {
  ...
  pmale2 <- findonor_and_snip_and_prs %>%
    ...
    #mutate(across(convert_to_integer, as.integer)) %>%
  pmale2 <- summary_plotter(pmale2, donor_descript, "blue")
  pmale2
}

time_series_length_plotter(data.male)
...
pfemale <- summary_plotter(pfemale, descript, "orange")
pfemale
...
if (!is.null(params$donor_specific_file)) {
  ...
  pfemale2 <- findonor_and_snip_and_prs %>%
    filter(donor %in% temp_donors) #%>%
  pfemale2 <- summary_plotter(pfemale2, donor_descript, "orange")
...
time_series_length_plotter(data.female)
rm(data.female2);
```
