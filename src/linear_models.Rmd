...
kable(descript)
#if (!is.null(donor_variables)) {
  kable(donor_descript %>% filter(Variable %in% c(names(donor_specific_variables), "one_deferral")))
#}
```
...
run_linear_model <- function(Id, Model, Pretty, Sex, stan_model, stan_parameters, pretty_variables) {
  ...
  if (out_of_sample_predictions) {
    ...
    if (skip_train) {
      ...
      results <- predict_new(pred.list, n.samples = params$iterations / 2)
  ...
  if (out_of_sample_predictions) {
    df <- create_result_dataframe(sl.val, prediction_matrix, Hb_cutoff, metric="mean")  
  } else {
