...
if ("gender" %in% colnames(data)) {
  data <- data %>% mutate(gender=ifelse(gender=="Men", 0, 1))
}
pdata <- data %>%
...
```{r, results="asis", echo=FALSE}
cat(sprintf("The %s set contains %i donations from %i donors.", params$gender, nrow(data), n_distinct(data$donor)))
...
message(sprintf("Maximum rhat is %f for %s lmm\n", max(rhats), params$gender))

v <- 0
if (params$gender == "both") {
  message("Here inside")
  message(sprintf("Length of x_test$gender: %i, mean: %f\n", length(stan.preprocessed.lmm$x_test$gender), mean(stan.preprocessed.lmm$x_test$gender)))
  # For some reason I have to do this inside the 'if' expression
  v <- ifelse(stan.preprocessed.lmm$x_test$gender == 0, params$Hb_cutoff_male, params$Hb_cutoff_female)
}
message("Here")
# If gender=="both then Hb_cutoff is vector, otherwise it is scalar
Hb_cutoff <- case_when(params$gender=="male" ~ params$Hb_cutoff_male, 
                       params$gender=="female" ~ params$Hb_cutoff_female,
                       TRUE ~ v)
message("Here2")
...
#     stan.preprocessed.lmm$x_test$gender == "male"   & original_Hb < params$Hb_cutoff_male   ~ 1,
#     stan.preprocessed.lmm$x_test$gender == "female" & original_Hb < Hb_cutoff_female ~ 1,
#     TRUE ~ 0
#   )
# } else {
...
id <- sprintf("%s-lmm", params$gender)
t <- tibble(Id=id, Model="Linear mixed model", Gender=params$gender, 
...
save(val, file = sprintf("%s/raw_result_%s_lmm_%s.rdata", rawresultdir, params$gender, fit_id))
...
v <- 0
if (params$gender == "both") {
  message(sprintf("Length of x_test$gender: %i, mean: %f\n", length(stan.preprocessed.dlmm$x_test$gender), mean(stan.preprocessed.dlmm$x_test$gender)))
  # For some reason I have to do this inside the 'if' expression
  v <- ifelse(stan.preprocessed.dlmm$x_test$gender == 0, params$Hb_cutoff_male, params$Hb_cutoff_female)
}
...
id <- sprintf("%s-dlmm", params$gender)
t <- tibble(Id=id, Model="Dynamic linear mixed model", Gender=params$gender, 
            MAE=val$mae, RMSE=val$rmse, 
            MAE2=val$mae2, RMSE2=val$rmse2)
t <- bind_cols(c(t, val$roc_ci, val$pr_ci, val$f1_ci))
summary_rows[[length(summary_rows)+1]] <- t
...
save(val, file = sprintf("%s/raw_result_%s_dlmm_%s.rdata", rawresultdir, params$gender, fit_id))
