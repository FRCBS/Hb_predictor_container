...
#message(params$hyperparameters)

# if (TRUE) {
#   quick <- TRUE
#   if (quick) {
#     svm_grid <- expand.grid(degree = c(3:5),
#                             scale = c(0.1, 1),
#                             C = c(1, 2))
#   } else {
#     svm_grid <- expand.grid(degree = c(3:5),
...
#                             C = seq(1,5,by=0.25))  
#   }
#   svm_hyperparameters <- learn_hyperparameters(train, "svmPoly", svm_grid, number_of_cores)
# } else {
...
train <- train %>% mutate(across(c(warm_season, previous_Hb_def), as.integer))   ## POISTA, vain testatusta varten 
validate <- validate %>% mutate(across(c(warm_season, previous_Hb_def), as.integer))   ## POISTA, vain testatusta varten 
...
#svmFit_roc <- caret::train(x = train %>% select(-Hb_deferral), y = train %>% pull(Hb_deferral),
                 ...
                 metric="ROC")
saveRDS(svmFit_roc, sprintf("/tmp/svm-fit-%s.rds", params$sex))
...
if (use_pdf) {     
  ...
  res <- compute_shap_values_shapr(svmFit_roc, validate, variables) 
...
svm_variable_importances <- rownames_to_column(svmFit_rocImp$importance)  # Cleaner than above
message("test2")
...
train$pred <- predict(svmFit_roc, newdata = train)
prediction_vl <- predict(svmFit_roc, newdata = validate)
prediction_probs <- as.data.frame(predict(svmFit_roc, newdata = validate, type="prob"))
...
# No variable importance plot for SVM, will be replaced by shap values
if (!is.null(params$effect_size_table_file)) {
  write_csv(validate, params$effect_size_table_file)   # Pass the table to the web app as well
}
```
...
message(nrow(validate))
message(class(prediction_probs))
