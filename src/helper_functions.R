...
filter_based_on_number_of_donations <- function(df, hlen) {
  if (!is.null(hlen)) {
    leq <- FALSE
    if (hlen < 0) {
      leq <- TRUE
...
stan_preprocess_new <- function(df, normalize = TRUE, Hb_index = 1, tolag = NULL,
                                donor_variables = NULL, test_data = TRUE, hlen = NULL) {
  ...
  df <- df %>% filter(!is.na(Hb_first), !is.na(days_to_previous_fb),
                      !is.na(Hb), first_event == FALSE,
...
stan_preprocess_icp_new <- function(df, Hb_index = 1, frac = NULL, normalize = TRUE, hlen = NULL, 
