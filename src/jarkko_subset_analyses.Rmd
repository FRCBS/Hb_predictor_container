...
#old.vars1 <- setdiff(descript$Variable, c("donor", "Hb", "previous_Hb", "Hb_deferral"))
old.vars1 <- descript %>% filter(!Variable %in% c("donor", "Hb", "previous_Hb", "Hb_deferral")) %>% pull(Pretty)   # Convert to pretty names
# old.vars1.icp <- c(c("days_to_previous_fb","Age","Previous_Hb_deferral","Year","Warm_season",
#                      "Consecutive_Hb_deferrals","Recent_donations","Recent_Hb_deferrals", "Hour","Previous_Hb") ,
#                    c("Age_first", "Year_first", "warm_season_first", "hour_first"))
#old.vars1.icp <- setdiff(descript$Variable, c("donor", "Hb", "Hb_first", "Hb_deferral"))
old.vars1.icp <- descript %>% filter(!Variable %in% c("donor", "Hb", "Hb_first", "Hb_deferral")) %>% pull(Pretty)   # Convert to pretty names
old.vars1.icp <- c(old.vars1.icp,                    
                   c("First age", "First year", "First warm season", "First hour"))
if (!is.null(donor_variables)) {
  vars1 <- c(old.vars1, pretty_donor_variables)
  vars1.icp <- c(old.vars1.icp, pretty_donor_variables)
} else {
  vars1 <- old.vars1
...
#  select(donor, Hb, days_to_previous_fb, age, previous_Hb_def, year, warm_season, consecutive_deferrals,
#         recent_donations, recent_deferrals, hour, previous_Hb, Hb_first, Hb_deferral) %>%
...
pmale <- data.male %>%
          keep(is.numeric) %>%
          gather() %>%
          mutate(key = factor(key, levels=descript$Variable))  %>% # Don't sort alphabetically
          ggplot(aes(value)) +
...
error_rows[[length(error_rows)+1]] <- list(Model="Male, linear mixed model", MAE=val1.m$mae, RMSE=val1.m$rmse, 
                                           MAE2=val1.m$mae2, RMSE2=val1.m$rmse2, AUC=val1.m$roc_auc, AUPR=val1.m$pr_auc)
...
error_rows[[length(error_rows)+1]] <- list(Model="Female, linear mixed model", MAE=val1.f$mae, RMSE=val1.f$rmse, 
                                        MAE2=val1.f$mae2, RMSE2=val1.f$rmse2, AUC=val1.f$roc_auc, AUPR=val1.f$pr_auc)
...
error_rows[[length(error_rows)+1]] <- list(Model="Male, dynamic linear mixed model", MAE=val1.m.icp$mae, RMSE=val1.m.icp$rmse, 
                                        MAE2=val1.m.icp$mae2, RMSE2=val1.m.icp$rmse2, AUC=val1.m.icp$roc_auc, AUPR=val1.m.icp$pr_auc)
...
error_rows[[length(error_rows)+1]] <- list(Model="Female, dynamic linear mixed model", MAE=val1.f.icp$mae, RMSE=val1.f.icp$rmse, 
                                        MAE2=val1.f.icp$mae2, RMSE2=val1.f.icp$rmse2, AUC=val1.f.icp$roc_auc, AUPR=val1.f.icp$pr_auc)

save(val1.f.icp, file = paste(rawresultdir, "raw_result_female_icp", fit_id, ".rdata", sep = ''))
...
write_csv(error, paste(rawresultdir, "errors_", fit_id, ".csv", sep = ''))
